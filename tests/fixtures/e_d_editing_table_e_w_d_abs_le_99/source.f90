program ede_table_e_w_d_abs_le_99
  implicit none
! rule: S13.7.2.3.3-005
! covers: table13_1_E_w_d_abs_le_99
  integer :: checks
  real :: value
  ...
  value = 1.0
  field = '##########'
  write(field,'(SS,E10.3)') value
  call expect_text(field(3:6), '.100', 'significand')
  call expect_either(field(7:10), 'E+01', '+001', 'exponent')
  if (checks /= 2) then
    write(*,'(a)') 'EDE:table_e_w_d_abs_le_99:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING TABLE E W D ABS LE 99 OK'
contains
...
end program ede_table_e_w_d_abs_le_99
