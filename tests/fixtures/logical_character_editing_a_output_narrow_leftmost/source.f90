program lce_a_output_narrow_leftmost
  implicit none
! rule: S13.7.4-004
! covers: output-narrow-field-leftmost
  integer :: checks
  ...
  value = 'wxyz'
  field = '##'
  write(field,'(A2)') value
  call expect_text(field, 'wx', 'a2-narrow')
  if (checks /= 1) then
    write(*,'(a)') 'LCE:a_output_narrow_leftmost:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING A OUTPUT NARROW LEFTMOST OK'
contains
...
end program lce_a_output_narrow_leftmost
