! rule: S16.9.98-002
! covers: IACHAR-result-integer
! covers: IACHAR-result-default-kind-without-KIND
! covers: IACHAR-result-kind-from-KIND
program i169l_iachar_result_characteristics
  implicit none
  interface type_code
    procedure type_code_integer, type_code_real
  end interface
  integer, parameter :: ik = merge(8, 4, kind(0) /= 8)
  integer :: checks
  checks = 0
  call require('IACHAR result is integer', type_code(iachar('A')) == 1, checks)
  call require('IACHAR without KIND is default integer kind', kind(iachar('A')) == kind(0), checks)
  call require('IACHAR result kind follows KIND', kind(iachar('A', kind=ik)) == ik, checks)
  if (checks /= 3) error stop
  write(*,'(a)') 'INTRINSICS 16.9 L IACHAR RESULT CHARACTERISTICS OK'
contains
  subroutine require(label, condition, checks)
    character(len=*), intent(in) :: label
    logical, intent(in) :: condition
    integer, intent(inout) :: checks
    if (.not. condition) then
      write(*,'(a)') label
      error stop
    end if
    checks = checks + 1
  end subroutine require
  integer function type_code_integer(x)
    integer, intent(in) :: x
    type_code_integer = 1
  end function type_code_integer
  integer function type_code_real(x)
    real, intent(in) :: x
    type_code_real = 2
  end function type_code_real
end program i169l_iachar_result_characteristics
