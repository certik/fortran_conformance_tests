program lce_a_omitted_width_output
  implicit none
! rule: S13.7.4-002
! covers: A-omitted-width-item-length
  integer :: checks
  ...
  field = '##'
  write(field,'(A)') value
  call expect_text(field, 'xy', 'a-omitted')
  if (checks /= 1) then
    write(*,'(a)') 'LCE:a_omitted_width_output:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING A OMITTED WIDTH OUTPUT OK'
contains
...
end program lce_a_omitted_width_output
