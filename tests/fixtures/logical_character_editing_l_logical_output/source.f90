program lce_l_logical_output
  implicit none
! rule: S13.7.3-001
! covers: L-logical-output
  integer :: checks
  logical :: value
  ...
  value = .true.
  field = '##'
  write(field,'(L2)') value
  call expect_text(field, ' T', 'l-logical-item')
  if (checks /= 1) then
    write(*,'(a)') 'LCE:l_logical_output:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING L LOGICAL OUTPUT OK'
contains
...
end program lce_l_logical_output
