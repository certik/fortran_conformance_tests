program fe_finite_output_sign
  implicit none
! rule: S13.7.2.3.2-012
! covers: F-finite-output-sign
  integer :: checks
  real :: value
  ...
  value = -3.0
  field = '#####'
  write(field,'(SS,F5.1)') value
  call expect_text(field, ' -3.0', 'minus-sign')
  if (checks /= 1) then
    write(*,'(a)') 'FE:finite_output_sign:check-total'
    error stop 1
  end if
  write(*,'(a)') 'F EDITING FINITE OUTPUT SIGN OK'
contains
...
end program fe_finite_output_sign
