! rule: S10.2.3.2-012
! covers: where-construct-selected-elements-assigned
...
program wm06
  ...
  if (actual(1) /= 101) then
    write(*,'(a)') 'WM:construct_selected_assigned:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= -702) then
    write(*,'(a)') 'WM:construct_selected_assigned:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= 103) then
    write(*,'(a)') 'WM:construct_selected_assigned:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= -704) then
    write(*,'(a)') 'WM:construct_selected_assigned:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= -705) then
    write(*,'(a)') 'WM:construct_selected_assigned:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 106) then
    write(*,'(a)') 'WM:construct_selected_assigned:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:construct_selected_assigned:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING CONSTRUCT SELECTED ASSIGNED OK'
end program wm06
