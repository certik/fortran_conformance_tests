! rule: S10.2.3.2-001
! covers: top-level-where-construct-initial-pending-region
...
program wm08
  ...
  if (actual(1) /= 101) then
    write(*,'(a)') 'WM:construct_initial_pending_region:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= 202) then
    write(*,'(a)') 'WM:construct_initial_pending_region:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= 303) then
    write(*,'(a)') 'WM:construct_initial_pending_region:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= 104) then
    write(*,'(a)') 'WM:construct_initial_pending_region:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= 305) then
    write(*,'(a)') 'WM:construct_initial_pending_region:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 106) then
    write(*,'(a)') 'WM:construct_initial_pending_region:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:construct_initial_pending_region:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING CONSTRUCT INITIAL PENDING REGION OK'
end program wm08
