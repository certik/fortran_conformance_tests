! rule: S10.2.3.2-012
! covers: corresponding-element-values
...
program wm04
  ...
  if (actual(-2) /= 501) then
    write(*,'(a)') 'WM:corresponding_element_values:final--2'
    error stop
  end if
  checks=checks+1
  if (actual(-1) /= -901) then
    write(*,'(a)') 'WM:corresponding_element_values:final--1'
    error stop
  end if
  checks=checks+1
  if (actual(0) /= 503) then
    write(*,'(a)') 'WM:corresponding_element_values:final-0'
    error stop
  end if
  checks=checks+1
  ...
  if (actual(3) /= -897) then
    write(*,'(a)') 'WM:corresponding_element_values:final-3'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:corresponding_element_values:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING CORRESPONDING ELEMENT VALUES OK'
end program wm04
