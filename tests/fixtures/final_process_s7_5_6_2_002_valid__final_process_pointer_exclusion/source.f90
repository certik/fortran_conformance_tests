...
call reset_log()
allocate(target,stat=status)
if (status/=0) error stop 111
if (.not.allocated(target)) error stop 112
allocate(owner,stat=status)
if (status/=0) error stop 111
if (.not.allocated(owner)) error stop 112
target%token=31
owner%token=11
owner%link=>target
if (.not.allocated(owner)) error stop 113
deallocate(owner,stat=status)
if (status/=0) error stop 114
if (allocated(owner)) error stop 115
if (.not.allocated(target)) error stop 122
call expect_log([10],[11])
call reset_log()
if (.not.allocated(target)) error stop 113
deallocate(target,stat=status)
if (status/=0) error stop 114
if (allocated(target)) error stop 115
call expect_log([20],[31])
end program p
