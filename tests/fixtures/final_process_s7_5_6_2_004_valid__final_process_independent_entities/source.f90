...
call reset_log()
allocate(left,stat=status)
if (status/=0) error stop 111
if (.not.allocated(left)) error stop 112
allocate(right,stat=status)
if (status/=0) error stop 111
if (.not.allocated(right)) error stop 112
left%token=17
right%token=19
if (.not.allocated(left)) error stop 113
if (.not.allocated(right)) error stop 113
deallocate(left,right,stat=status)
if (status/=0) error stop 114
if (allocated(left)) error stop 115
if (allocated(right)) error stop 115
call expect_log([20,20],[17,19])
end program p
