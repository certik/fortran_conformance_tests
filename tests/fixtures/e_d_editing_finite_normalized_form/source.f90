program ede_finite_normalized_form
  implicit none
! rule: S13.7.2.3.3-004
! covers: E-D-finite-normalized-form
  integer :: checks
  real :: value
  ...
  call expect_text(field(3:10), '.100E+01', 'slice')
  if (checks /= 1) then
    write(*,'(a)') 'EDE:finite_normalized_form:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING FINITE NORMALIZED FORM OK'
contains
...
end program ede_finite_normalized_form
