program lce_a_explicit_width_output
  implicit none
! rule: S13.7.4-002
! covers: A-explicit-width
  integer :: checks
  ...
  call expect_text(field, '  xy', 'a4-wide')
  if (checks /= 1) then
    write(*,'(a)') 'LCE:a_explicit_width_output:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING A EXPLICIT WIDTH OUTPUT OK'
contains
...
end program lce_a_explicit_width_output
