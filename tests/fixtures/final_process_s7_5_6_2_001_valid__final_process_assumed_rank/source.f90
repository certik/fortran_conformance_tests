...
call reset_log()
allocate(scalar,stat=status)
if (status/=0) error stop 111
if (.not.allocated(scalar)) error stop 112
scalar%token=11
if (.not.allocated(scalar)) error stop 113
deallocate(scalar,stat=status)
if (status/=0) error stop 114
if (allocated(scalar)) error stop 115
call expect_log([50,60],[0,11])
call reset_log()
...
call expect_log([50,60,60],[1,17,19])
call reset_log()
allocate(matrix(0,2),stat=status)
if (status/=0) error stop 111
