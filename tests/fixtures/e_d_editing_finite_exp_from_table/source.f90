program ede_finite_exp_from_table
  implicit none
! rule: S13.7.2.3.3-004
! covers: E-D-finite-exp-from-table
  integer :: checks
  real :: value
  ...
  call expect_text(field(7:10), 'E+01', 'slice')
  if (checks /= 1) then
    write(*,'(a)') 'EDE:finite_exp_from_table:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING FINITE EXP FROM TABLE OK'
contains
...
end program ede_finite_exp_from_table
