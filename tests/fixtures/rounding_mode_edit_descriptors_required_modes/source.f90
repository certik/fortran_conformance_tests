program rm_edit_descriptors_required_modes
  implicit none
! rule: S13.8.8-001
! covers: RU-sets-up
! covers: RD-sets-down
! covers: RZ-sets-zero
! covers: RN-sets-nearest
! covers: RC-sets-compatible
  integer :: checks
  ...
  if (checks /= 5) then
    write(*,'(a)') 'RM:edit_descriptors_required_modes:check-total'
    error stop 1
  end if
  write(*,'(a)') 'ROUNDING MODE EDIT DESCRIPTORS REQUIRED MODES OK'
contains
...
end program rm_edit_descriptors_required_modes
