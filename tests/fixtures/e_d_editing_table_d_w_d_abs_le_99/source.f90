program ede_table_d_w_d_abs_le_99
  implicit none
! rule: S13.7.2.3.3-005
! covers: table13_1_D_w_d_abs_le_99
  integer :: checks
  real :: value
  ...
  value = 1.0
  field = '##########'
  write(field,'(SS,1P,D10.3)') value
  call expect_text(field(2:6), '1.000', 'significand')
  call expect_one_of(field(7:10), 'D+00', 'E+00', '+000', 'exponent')
  if (checks /= 2) then
    write(*,'(a)') 'EDE:table_d_w_d_abs_le_99:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING TABLE D W D ABS LE 99 OK'
contains
...
end program ede_table_d_w_d_abs_le_99
