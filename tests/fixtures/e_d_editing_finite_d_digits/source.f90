program ede_finite_d_digits
  implicit none
! rule: S13.7.2.3.3-004
! covers: E-D-finite-d-digits
  integer :: checks
  real :: value
  ...
  call expect_text(field(4:6), '100', 'slice')
  if (checks /= 1) then
    write(*,'(a)') 'EDE:finite_d_digits:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING FINITE D DIGITS OK'
contains
...
end program ede_finite_d_digits
