! rule: S10.2.3.2-012
! covers: where-stmt-unselected-elements-unchanged
...
program wm03
  ...
  if (actual(-2) /= 501) then
    write(*,'(a)') 'WM:where_stmt_unselected_unchanged:final--2'
    error stop
  end if
  checks=checks+1
  if (actual(-1) /= -901) then
    write(*,'(a)') 'WM:where_stmt_unselected_unchanged:final--1'
    error stop
  end if
  checks=checks+1
  if (actual(0) /= 503) then
    write(*,'(a)') 'WM:where_stmt_unselected_unchanged:final-0'
    error stop
  end if
  checks=checks+1
  ...
  if (actual(3) /= -897) then
    write(*,'(a)') 'WM:where_stmt_unselected_unchanged:final-3'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:where_stmt_unselected_unchanged:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING WHERE STMT UNSELECTED UNCHANGED OK'
end program wm03
