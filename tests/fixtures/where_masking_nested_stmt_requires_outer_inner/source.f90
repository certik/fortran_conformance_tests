! rule: S10.2.3.2-008
! covers: nested-where-stmt-requires-outer-and-inner-true
...
program wm19
  ...
  if (actual(1) /= 101) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= -802) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= -803) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= -804) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= -805) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 106) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:nested_stmt_requires_outer_inner:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING NESTED STMT REQUIRES OUTER INNER OK'
end program wm19
