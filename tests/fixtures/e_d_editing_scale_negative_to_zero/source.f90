program ede_scale_negative_to_zero
  implicit none
! rule: S13.7.2.3.3-006
! covers: E-D-scale-negative-to-zero-normalization
  integer :: checks
  real :: value
  ...
  call expect_text(field(3:10), '.010E+02', 'scaled')
  if (checks /= 1) then
    write(*,'(a)') 'EDE:scale_negative_to_zero:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING SCALE NEGATIVE TO ZERO OK'
contains
...
end program ede_scale_negative_to_zero
