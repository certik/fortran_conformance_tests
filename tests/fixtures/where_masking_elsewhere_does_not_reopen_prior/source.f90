! rule: S10.2.3.2-005
! covers: elsewhere-does-not-reopen-prior-branches
...
program wm13
  ...
  if (actual(1) /= 101) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= 202) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= 303) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= 104) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= 305) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 106) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:elsewhere_does_not_reopen_prior:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING ELSEWHERE DOES NOT REOPEN PRIOR OK'
end program wm13
