program lce_l_lowercase_t
  implicit none
! rule: S13.7.3-003
! covers: lowercase-t-true
  integer :: checks
  ...
  field = ' t'
  guard = 0
  call mark_logical('lower-t', value, guard, .false.)
  call expect_pre_logical(value, .false., guard, 'lower-t')
  read(field,'(L2)') value
  call expect_logical(value, .true., 'lower-t')
  if (checks /= 2) then
    write(*,'(a)') 'LCE:l_lowercase_t:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING L LOWERCASE T OK'
contains
