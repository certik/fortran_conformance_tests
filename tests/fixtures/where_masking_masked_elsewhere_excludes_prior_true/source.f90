! rule: S10.2.3.2-004
! covers: masked-elsewhere-leaves-prior-true-excluded
...
program wm10
  ...
  if (actual(1) /= 101) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= 202) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= 303) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= 104) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= 305) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 106) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:masked_elsewhere_excludes_prior_true:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING MASKED ELSEWHERE EXCLUDES PRIOR TRUE OK'
end program wm10
