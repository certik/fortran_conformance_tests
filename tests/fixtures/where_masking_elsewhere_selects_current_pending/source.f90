! rule: S10.2.3.2-005
! covers: elsewhere-selects-current-pending
...
program wm12
  ...
  if (actual(1) /= 101) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= 202) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= 303) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= 104) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= 305) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 106) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:elsewhere_selects_current_pending:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING ELSEWHERE SELECTS CURRENT PENDING OK'
end program wm12
