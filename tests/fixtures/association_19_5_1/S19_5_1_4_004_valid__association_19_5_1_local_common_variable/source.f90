! rule: S19.5.1.4-004
! covers: local-common-variable-hides-host
! evidence: effect
! standard: f2023
! oracle-basis: standard
program association_local_common_variable
  implicit none
  ...
  checks = 0
  call inner()
  call expect_equal(observed, 308, 'local-common-variable-hides-host local value')
  call expect_equal(x, 207, 'local-common-variable-hides-host host unchanged')
  call expect_equal(checks, 2, 'check count before completion')
  write(*,'(a)') 'ASSOCIATION 19.5.1 LOCAL COMMON VARIABLE OK'
contains
  subroutine inner()
    implicit none
    integer :: x
...
end program association_local_common_variable
