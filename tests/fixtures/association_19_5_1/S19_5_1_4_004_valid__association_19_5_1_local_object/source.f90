! rule: S19.5.1.4-004
! covers: local-object-declaration-hides-host
! evidence: effect
! standard: f2023
! oracle-basis: standard
program association_local_object
  implicit none
  ...
  checks = 0
  call inner()
  call expect_equal(observed, 302, 'local-object-declaration-hides-host local value')
  call expect_equal(x, 201, 'local-object-declaration-hides-host host unchanged')
  call expect_equal(checks, 2, 'check count before completion')
  write(*,'(a)') 'ASSOCIATION 19.5.1 LOCAL OBJECT OK'
contains
  subroutine inner()
    implicit none
    integer :: x
...
end program association_local_object
