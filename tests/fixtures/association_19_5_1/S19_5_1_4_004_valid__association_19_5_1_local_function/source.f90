! rule: S19.5.1.4-004
! covers: local-function-name-hides-host
! evidence: effect
! standard: f2023
! oracle-basis: standard
program association_local_function
  implicit none
  ...
  checks = 0
  call inner()
  call expect_equal(observed, 303, 'local-function-name-hides-host local value')
  call expect_equal(x, 202, 'local-function-name-hides-host host unchanged')
  call expect_equal(checks, 2, 'check count before completion')
  write(*,'(a)') 'ASSOCIATION 19.5.1 LOCAL FUNCTION OK'
contains
  subroutine inner()
    implicit none
    integer :: x, n
...
end program association_local_function
