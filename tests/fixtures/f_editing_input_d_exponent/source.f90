program fe_input_d_exponent
  implicit none
! rule: S13.7.2.3.2-003
! covers: F-input-D-exponent-form
  integer :: checks
  ...
  field = '1D+1'
  value = -99.0
  call expect_real(value, -99.0, 'sentinel-d-exponent')
  read(field,'(F4.0)') value
  call expect_real(value, 10.0, 'd-exponent')
  if (checks /= 2) then
    write(*,'(a)') 'FE:input_d_exponent:check-total'
    error stop 1
  end if
  write(*,'(a)') 'F EDITING INPUT D EXPONENT OK'
contains
...
end program fe_input_d_exponent
