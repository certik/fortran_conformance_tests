! rule: S7.5.10-008
! covers: typed-mold-null-unallocated
...
program structure_constructor_typed_mold_null_allocatable_effect
  implicit none
  integer :: checks
  type :: record
    integer :: lead
  ...
  if (checks /= 4) then
    write(*,'(a)') 'DTSC:typed_mold_null_allocatable:check-total'
    error stop
  end if
  write(*,'(a)') 'STRUCTURE CONSTRUCTOR TYPED MOLD NULL ALLOCATABLE OK'
contains
  subroutine observe(obj)
    type(record), intent(in) :: obj
    if (obj%lead /= 101) then
      write(*,'(a)') 'DTSC:typed_mold_null_allocatable:lead-component'
      error stop
    end if
    checks=checks+1
    if (obj%swapped /= 103) then
      write(*,'(a)') 'DTSC:typed_mold_null_allocatable:swapped-component'
      error stop
    end if
    checks=checks+1
    if (obj%stamp /= -9051) then
      write(*,'(a)') 'DTSC:typed_mold_null_allocatable:default-stamp'
      error stop
    end if
    checks=checks+1
    if (.not. (.not. allocated(obj%vector))) then
      write(*,'(a)') 'DTSC:typed_mold_null_allocatable:typed-mold-null-unallocated'
      error stop
    end if
    checks=checks+1
