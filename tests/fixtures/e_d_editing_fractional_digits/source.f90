program ede_fractional_digits
  implicit none
! rule: S13.7.2.3.3-001
! covers: E-D-fractional-digits
  integer :: checks
  real :: value
  ...
  value = 1.0
  field = '#########'
  write(field,'(SS,1P,E9.2E2)') value
  call expect_text(field, ' 1.00E+00', 'field')
  if (checks /= 1) then
    write(*,'(a)') 'EDE:fractional_digits:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING FRACTIONAL DIGITS OK'
contains
...
end program ede_fractional_digits
