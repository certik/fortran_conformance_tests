program ede_exponent_digit_count
  implicit none
! rule: S13.7.2.3.3-001
! covers: E-D-exponent-digit-count
  integer :: checks
  real :: value
  ...
  value = 0.5
  field = '###########'
  write(field,'(SS,2P,E11.3E3)') value
  call expect_text(field, ' 50.00E-002', 'field')
  if (checks /= 1) then
    write(*,'(a)') 'EDE:exponent_digit_count:check-total'
    error stop 1
  end if
  write(*,'(a)') 'E D EDITING EXPONENT DIGIT COUNT OK'
contains
...
end program ede_exponent_digit_count
