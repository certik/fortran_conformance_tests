program lce_a_character_item
  implicit none
! rule: S13.7.4-001
! covers: A-character-item
  integer :: checks
  ...
  call expect_text(field, 'xy', 'a-character-item')
  if (checks /= 1) then
    write(*,'(a)') 'LCE:a_character_item:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING A CHARACTER ITEM OK'
contains
...
end program lce_a_character_item
