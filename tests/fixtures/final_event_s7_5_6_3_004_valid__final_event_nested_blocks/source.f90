...
type(item) :: inner
inner%token=19
! checkpoint: nested-inner-inside
call expect_log([integer ::],[integer ::])
end block
! checkpoint: nested-between
call expect_log([10],[19])
end block
! checkpoint: nested-after
call expect_log([10,10],[19,17])
call expect_before(10,19,10,17)
end program p
