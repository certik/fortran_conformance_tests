program lce_g_logical_output
  implicit none
! rule: S13.7.3-001
! covers: G-logical-editing-permitted
  integer :: checks
  logical :: value
  ...
  call expect_text(field, 'F ', 'g0-logical')
  if (checks /= 1) then
    write(*,'(a)') 'LCE:g_logical_output:check-total'
    error stop 1
  end if
  write(*,'(a)') 'LOGICAL CHARACTER EDITING G LOGICAL OUTPUT OK'
contains
...
end program lce_g_logical_output
