! rule: S10.2.3.2-006
! covers: nested-construct-restores-outer-pending
...
program wm18
  ...
  if (actual(1) /= 1101) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:final-1'
    error stop
  end if
  checks=checks+1
  if (actual(2) /= 1202) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:final-2'
    error stop
  end if
  checks=checks+1
  if (actual(3) /= 303) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:final-3'
    error stop
  end if
  checks=checks+1
  if (actual(4) /= 304) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:final-4'
    error stop
  end if
  checks=checks+1
  if (actual(5) /= 1205) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:final-5'
    error stop
  end if
  checks=checks+1
  if (actual(6) /= 1106) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:final-6'
    error stop
  end if
  checks=checks+1
  if (checks /= 6) then
    write(*,'(a)') 'WM:nested_construct_restores_outer_pending:check-total'
    error stop
  end if
  write(*,'(a)') 'WHERE MASKING NESTED CONSTRUCT RESTORES OUTER PENDING OK'
end program wm18
