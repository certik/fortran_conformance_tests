"""Tests for generated Clause 16.9.18-16.9.21 intrinsic fixtures."""
...
import generate_intrinsics_16_9_c_fixtures as generated


class Intrinsics169CFixturesTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_generated_case_inventory_and_metadata(self):
        ...
        self.assertEqual(sum(len(spec["facets"]) for spec in self.specs.values()), 45)
        self.assertEqual(sum(len(spec["mutations"]) for spec in self.specs.values()), 46)
    ...
    def test_sources_are_bounded_and_source_derived(self):
        required_tokens = {
            "asinh_elemental": ["associate(result => asinh(x))", "rank(result) /= 1", "size(result) /= 3"],
            "asinh_arguments": ["real_result = asinh(real_x)", "complex_result = asinh(complex_x)"],
            "asinh_complex_range": ["aimag(y) < -pi / 2.0", "y = asinh(x)"],
            "atan_arguments": ["y_real_result = atan(y, x)", "same_kind_result = atan(y, x)",
                               "boundary_result = atan(boundary_y, boundary_x)", "complex_arg = atan(z)",
                               "[0.0, 1.0, 1.0]"],
            "atan_same_as_atan2": ["observed = atan(y, x)", "reference = atan2(y, x)", "observed /= reference"],
            "atan_one_arg_range": ["real_result = atan(1.4)", "complex_result = atan(cmplx(1.4, 0.25))"],
            "associated_host_instance": ["recursive subroutine host", "associated(carried, inner)", "associated(pcur, dummy)"],
            "associated_data_cases": ["array_target(1:0)", "character(len=0)", "array_target(2:4)"],
            "associated_pointer_cases": ["nullify(q_scalar)", "associated(p_array, q_array)", "zero_chars"],
        }
