"""Regression checks for the Clause 12.7-12.9 WAIT/position/FLUSH packet."""
...
import generate_io_wait_position_flush_12_7_12_9_fixtures as generated


class IoWaitPositionFlushFixturesTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_owned_fixture_set_and_metadata(self):
        ...
        self.assertEqual(sum(len(spec["facets"]) for spec in self.specs.values()), 34)
        for name, spec in self.specs.items():
            ...
            self.assertEqual(case.meta.evidence, spec.get("evidence", "effect"))
        ...
        for section in generated.CATALOGUES:
            ...
            for req in catalogue["requirements"]:
                ...
                if covered.get(req["id"]):
                    self.assertIn("WAIT/position/FLUSH fixture implementation:", req["oracle"])

    def test_valid_sources_have_guards_and_portable_status_policy(self):
        for name, spec in self.specs.items():
            ...
            for match in re.finditer(r"^\s*read\s*\(.*\)\s*([A-Za-z]\w*)\s*$", source, re.M | re.I):
                prefix = source[:match.start()].splitlines()[-4:]
                self.assertTrue(any(f"call check_" in line and match.group(1) in line for line in source[match.end():].splitlines()[:8])
                                or "asynchronous='yes'" in match.group(0).lower())
                self.assertTrue(any(re.match(r"\s*" + re.escape(match.group(1)) + r"\s*=", line, re.I)
                                    for line in prefix), (name, match.group(0)))
    ...
    def test_mutants_are_unique_conforming_feature_changes(self):
        ...
        self.assertGreaterEqual(feature_count, 32)
