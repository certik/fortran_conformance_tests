"""Regression checks for the Clause 12.5.6.3-.11 OPEN specifier fixture packet."""
...
import generate_io_open_specifiers_12_5_6_3_fixtures as generated


class IoOpenSpecifiersFixturesTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        for section in generated.CATALOGUES:
            ...
            for req in catalogue["requirements"]:
                ...
                if covered.get(req["id"]):
                    self.assertIn("OPEN-specifier fixture implementation:", req["oracle"])
    ...
    def test_sources_have_expected_guard_counts_and_portable_iostat_policy(self):
        for name, spec in self.specs.items():
            ...
            self.assertNotIn("status='old', file=", source.lower())
    ...
    def test_mutants_are_unique_conforming_feature_changes(self):
        ...
        self.assertGreaterEqual(feature_count, 16)
