"""Runtime fixture checks for Fortran 2023 IEEE 17.1-17.3 fixtures."""
...
import generate_ieee_17_1_17_3_fixtures as generated


class Ieee171173FixturesTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_owned_manifests_are_discoverable_and_bound(self):
        ...
        manifests = [path for path in self.files if path.name == "fixture.json"]
        self.assertEqual(len(manifests), len(generated.CASES))
    ...
    def test_sources_use_guarded_ieee_observations(self):
        guarded = [
            "exception_arrays_status", "flag_lifecycle", "exception_overflow", "exception_divide",
            "exception_invalid_sqrt", "exception_underflow_inexact", "relational_invalid_table",
            "masking_no_extra_operations", "procedure_flag_semantics",
        ]
        for spec in self.specs.values():
            source = spec["source"]
            source.encode("ascii")
            ...
            self.assertNotIn("UNSUPPORTED OK", source)
            self.assertNotIn("use, intrinsic :: ieee_exceptions", source.lower())
            self.assertNotIn("use, intrinsic :: ieee_features", source.lower())
            if spec["variant"] in guarded:
                self.assertIn("ieee_support", source.lower())
                self.assertRegex(source.lower(), r"ieee_support_(flag|datatype|nan|halting)")
                self.assertIn("SUPPORTED OK", source)
            if "ieee_get_flag" in source:
                self.assertIn("ieee_set_halting_mode", source)
            if "one / zero" in source or "sqrt(negative)" in source:
    ...
    def test_mutation_plans_bind_complete_parent_sources(self):
        ...
        for spec in self.specs.values():
            ...
            self.assertTrue(spec["probes"], spec["variant"])
            mutated_hashes = set()
            for probe in spec["probes"]:
                self.assertIn(probe["facet"], spec["facets"])
        ...
        self.assertEqual(covered, {facet for spec in self.specs.values() for facet in spec["facets"]})
        table_facets = {row[0] for row in generated.FEATURE_MUTATION_TABLE}
        self.assertEqual(table_facets, covered)
