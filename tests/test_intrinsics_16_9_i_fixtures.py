"""Tests for generated Clause 16.9.78-16.9.86 intrinsic fixtures."""
...
import generate_intrinsics_16_9_i_fixtures as generated


class Intrinsics169IFixturesTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_sources_are_bounded_and_source_derived(self):
        required_tokens = {
            "epsilon_characteristics": ["rank(epsilon(array_x))", "kind(epsilon(double_x))"],
            "epsilon_model": ["radix(default_x)", "digits(default_x)", "epsilon(double_x) == double_expected"],
            "erf_result_characteristics": ["kind(erf(double_x))", "shape(erf(xs))"],
            "erfc_result_characteristics": ["kind(erfc(double_x))", "shape(erfc(xs))"],
            "erfc_scaled_result_characteristics": ["kind(erfc_scaled(double_x))", "shape(erfc_scaled(xs))"],
            "exp_result_characteristics": ["kind(exp(double_scalar))", "kind(exp(double_complex))"],
            "exponent_model": ["real(radix(default_x)", "default_power_k + 1",
                               "exponent(double_power) == double_power_k + 1"],
            "exponent_result_kind": ["kind(exponent(result_x))", "alt_int_kind"],
            "exponent_ieee": ["ieee_value(0.0, ieee_positive_inf)", "ieee_value(0.0, ieee_quiet_nan)"],
            "extends_type_characteristics": ["kind(extends_type_of(c, p))", "rank(extends_type_of(c, p))",
                                             "alt_logical_kind"],
            "extends_type_unlimited_absent": ["extends_type_of(p, mold_absent)", "extends_type_of(a_absent, p)",
                                              "extends_type_of(a_pointer, p)"],
            "extends_type_extension_values": ["extends_type_of(g, p)", "extends_type_of(p, c)", "extends_type_of(s, c)"],
        }
